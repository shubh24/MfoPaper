...
%Moth_pos = initialization1(N, ess, dat);
...
while Iteration<Max_iteration+1
    ...
    % Bringing the moths if they are out of bounds 
    ...
    for i=1:size(Moth_pos,1)
		...
		dist = sum(Moth_pos(i,:) != sorted_population(i,:));
	  	dist = round(spiral3*dim);
		Moth_pos(i,:) = fly(Moth_pos(i,:), sorted_population(i,:), dist);
	    end
            
            if i>Flame_no % Update the position of the moth with respct to one flame
	    
		dist = sum(Moth_pos(i,:) != sorted_population(1,:));
	  	dist = round(spiral3*dim);
        	Moth_pos(i,:) = fly(Moth_pos(i,:), sorted_population(1,:), dist);
            end
    end
