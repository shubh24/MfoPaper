...
function [Elite_antlion_fitness,Elite_antlion_position,Convergence_curve]=ALO(N,Max_iter,lb,ub,dim,fobj,handles,value)
...
while Current_iter<Max_iter+1
    ...
    if Current_iter>2
        line([Current_iter-1 Current_iter], [Convergence_curve(Current_iter-1) Convergence_curve(Current_iter)],'Color','b')
        xlabel('Iteration');
    ...
    set(handles.itertext,'String', ['The current iteration is ', num2str(Current_iter)])
    set(handles.optimumtext,'String', ['The current optimal value is ', num2str(Elite_antlion_fitness)])
    if value==1
        hold on
        scatter(Current_iter*ones(1,N),All_fitness,'.','k')
    end
